...
struct sobject_s;
typedef struct sobject_s sobject;
...
#define WORKSPACESIZE (4000) /* Cells (4*bytes) */
...
void error2(object *symbol, char *string)
{
  pfl();
  pfstring(("Error: "));
  if (symbol == NULL)
...
object *closure(object *fname, object *function, object *args, object **env)
{
  ...
  if (params != NULL)
    error2(fname, ("has too few parameters"));
  if (args != NULL)
    error2(fname, ("has too many parameters"));
...
// Built-in procedure names - stored in
...
object *eval(object *form, object *env)
{
  ...
  if (symbolp(function))
  {
    ...
    if (name >= ENDFUNCTIONS)
      error2(fname, ("is not valid here"));
    if (nargs < lookupmin(name))
      error2(fname, ("has too few arguments"));
    if (nargs > lookupmax(name))
      error2(fname, ("has too many arguments"));
...
typedef void (*write_fp)(int);
typedef int (*available_fp)();
typedef int (*read_fp)();
...
object *read()
{
  ...
  if (item == (object *)DOT)
    return read();
  if (item == (object *)QUO)
    return cons(symbol(QUOTE), cons(read(), NULL));
  return item;
}
